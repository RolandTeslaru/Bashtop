...
namespace monitor::os::mac {
    ...
    void CpuReader::sample(CpuRawSample& out) {
        ...
        if(kernel_call_return != KERN_SUCCESS)
            throw CpuSampleException("Mach kernel call was NOT successfull. Returned: " + std::to_string(kernel_call_return));

        if(info == nullptr)
            throw CpuSampleException("Mach kernel info returned nullptr.");

        if(cpuCount == 0)
            throw CpuSampleException("Mach kernell returned wrong cpu count.");
