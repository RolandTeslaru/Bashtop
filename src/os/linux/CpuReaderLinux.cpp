...
namespace monitor::os::linux {
    ...
    void CpuReader::sample(CpuRawSample& out) {
        ...
        if (!f) {
            throw CpuSampleException("Failed to open /proc/stat");
        }

        out.per_core.clear();

        std::string line;
        bool foundTotalCpu = false;
        ...
        while (std::getline(f, line)) {
            ...
            if(iss.fail())
                throw CpuSampleException("Failed to parse /proc/stat cpu line: " + line);
            ...
            if (label == "cpu") {
                ...
                try {
                    ...
                    throw CpuSampleException("Failed to parse core index from label: " + label);
                }
            } else {
                break;
            }
        ...
        if (!foundTotalCpu) {
            throw CpuSampleException("Failed to find total 'cpu' line in /proc/stat");
        }

        if (f.bad()) {
            throw CpuSampleException("Error occurred while reading /proc/stat");
        }
